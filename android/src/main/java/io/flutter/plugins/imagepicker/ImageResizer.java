...
class ImageResizer {
   ...
   */
  String resizeImageIfNeeded(String imagePath, Double maxWidth, Double maxHeight, Double quality) {
    ...
    try {
      File scaledImage = resizedImage(imagePath, maxWidth, maxHeight,quality);
  ...
  private File resizedImage(String path, Double maxWidth, Double maxHeight, Double quality) throws IOException {
    ...
    int qual = (int) (quality.doubleValue() * 100.0);

    Bitmap scaledBmp = Bitmap.createScaledBitmap(bmp, width.intValue(), height.intValue(), false);
    ByteArrayOutputStream outputStream = new ByteArrayOutputStream();
    scaledBmp.compress(Bitmap.CompressFormat.JPEG, qual, outputStream);
